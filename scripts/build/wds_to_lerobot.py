...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=HAND_FRAMES,
        default="camera",
        help="camera: express state/action in the current frame's head camera (EgoSteer world==head cam); world: keep SLAM world frame",
    )
    parser.add_argument("--task_source", choices=TASK_SOURCES, default="dataset_name", help="Which meta field becomes the LeRobot task (tasks[0] is what lerobot_to_wds writes as dataset_name; the sentences stay in instructions/language)")
    ...
    parser.add_argument("--no_extrinsic", action="store_true", help="Drop the per-frame world->camera feature")

...
def resolve_job(defaults: dict, job: dict) -> dict:
    ...
    # tasks[0] is the dataset name downstream (lerobot_to_wds writes it as dataset_name): default to the job name,
...
def run_job(job: dict, *, steps: list[str] | None = None, dry_run: bool = False) -> dict:
    ...
    if dry_run:
        ...
        report["release_problems"] = check_release_consistency(job["release"], include_video=report["convert_config"]["include_video"], has_source_frames=bool(job.get("descriptor_manifest") or job.get("source_map")))
...
def run_batch(config: dict, *, jobs_filter: list[str] | None = None, steps: list[str] | None = None, dry_run: bool = False, stop_on_fail: bool = False, skip_done: bool = False, report_dir: str | None = None, helper: bool = False) -> dict:
    ...
    for job in jobs:
        ...
        if helper:
            ...
            if not (Path(job["output_dir"]) / "_wds_to_lerobot_work").exists():
                logger.info("[%s] helper: the primary has not started this job yet, skipping", job["name"])
                continue
